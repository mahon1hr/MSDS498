...
## FOLLOWING CODE ADDED BETWEEN V2 AND V3 BY T. MARASS
# Code to create dummy variable columns for each of the restaurant categories

list_cat = business['cat'].unique()
for i in list_cat:
    i = i + '_dum'
    business[i] = np.where(business['cat'] == i, True, False)

## PRECEEDING CODE ADDED BETWEEN V2 AND V3 BY T. MARASS
...
business_df.drop(business_df.iloc[:, 28:93], inplace=True, axis=1) 
...
# Convert "cat" and "postal_code" columns to binary columns
